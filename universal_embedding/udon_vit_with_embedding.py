...
from universal_embedding import udon_model
...
class ViTWithEmbedding(vit.ViT):
	...
	def __call__(
		...
		*,
		...
		project_feats: bool = True, #not used anymore: TODO: deprecate it
		...
		if not domain_agnostic:
				
			#teacher projection
			teacher_domain=domain

			teacher_embedd = Mlp(
				num_layers = len(self.teacher_dim),
				hidden_size_list = self.teacher_dim,
				name=f'teacher_projection_domain_{teacher_domain}',
			)(backbone_out)

			teacher_embedd /= jnp.linalg.norm(teacher_embedd, ord=2, axis=1, keepdims=True)

			outputs['embeddings']['teacher_embedd'] = teacher_embedd


		#universal projection
		student_domain=0 #only 1 student always

		universal_student_embedd = Mlp(
			num_layers = len(self.universal_student_dim),
			hidden_size_list = self.universal_student_dim,
			name=f'universal_student_projection_domain_{student_domain}',
		)(backbone_out)

			# l2 norm the embeddings again
		universal_student_embedd /= jnp.linalg.norm(universal_student_embedd, ord=2, axis=1, keepdims=True)

		outputs['embeddings']['universal_student_embedd'] = universal_student_embedd 
		...
		if not return_feats: # pass through classification layer #(can be replaced with if train or init?)
			...
			#classifier is always separate in UDON
			...
			universal_student_logits = nn.Dense(
				classifier_num_classes,
				use_bias=False,
				kernel_init=nn.initializers.lecun_uniform(),
				name=f'universal_student_output_projection_{classifier_domain}',
			)(outputs['embeddings']['universal_student_embedd'])
			weights_norms = jnp.linalg.norm(
				self.variables['params'][f'universal_student_output_projection_{classifier_domain}']['kernel'], axis=0
			)  # norms of class prototypes
			universal_student_logits /= weights_norms

			outputs['classifier']['universal_student_logits'] = universal_student_logits

			#teacher classifier

			teacher_logits = nn.Dense(
				classifier_num_classes,
				use_bias=False,
				kernel_init=nn.initializers.lecun_uniform(),
				name=f'teacher_output_projection_{classifier_domain}',
			)(outputs['embeddings']['teacher_embedd'])
			weights_norms = jnp.linalg.norm(
				self.variables['params'][f'teacher_output_projection_{classifier_domain}']['kernel'], axis=0
			)  # norms of class prototypes
			teacher_logits /= weights_norms

			outputs['classifier']['teacher_logits'] = teacher_logits
...
class ViTWithEmbeddingUdonModel(
		udon_model.UniversalEmbeddingUdonModel
):
	...
	def build_flax_model(self) -> nn.Module:
		...
		return ViTWithEmbedding(
				...
				#udon parameters
				universal_student_dim=self.config.model.universal_student_dim,
