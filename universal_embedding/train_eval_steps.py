...
def representation_fn_eval(
  ...
  if gather_to_host:
    ...
    for embed_type in embeddings_dict:
      #no need to do this for batch masks, they are the same across embeddings
      embeddings_dict[embed_type] = jax.lax.all_gather(embeddings_dict[embed_type], 'batch')

    batch = jax.lax.all_gather(batch, 'batch')

  return embeddings_dict, batch['batch_mask']