...
def train(
  ...
  #log the sampling weights here
  log_sample_weights_dict = {}

  #do this also once in the beggining?
  for dataset_name in sampler.sampling_weights:
    log_sample_weights_dict[f"sampling_weights/{dataset_name}"] = sampler.sampling_weights[dataset_name]

  #the logging here happens only every update sampler steps, so no problem with that
  writer.write_scalars(0, log_sample_weights_dict)
  ...
  for step in range(start_step + 1, total_steps + 1):
    ...
    with jax.profiler.StepTraceAnnotation('train', step_num=step):
      ...
      if config.get("update_sampler",False) and step % config.update_sampler_every_steps == 0:

        sampler.update_ds_indices(train_metrics,step)
        
        #log the sampling weights here
        log_sample_weights_dict = {}

        #do this also once in the beggining? TODO
        for dataset_name in sampler.sampling_weights:
          log_sample_weights_dict[f"sampling_weights/{dataset_name}"] = sampler.sampling_weights[dataset_name]

        #the logging here happens only every update sampler steps, so no problem with that
        writer.write_scalars(step, log_sample_weights_dict)
    ...
    if (step % log_eval_steps == 0) or (step == total_steps):
      ...
      if do_knn:
        ...
        with report_progress.timed('knn'):
          ...
          try:
            epoch_average_common_val_knn_top_1 = results['knn_results']['universal']['average:common:val_knn:top_1']
          except:
            epoch_average_common_val_knn_top_1 = results['knn_results']['universal']['average:separate:val_knn:top_1']
